...
def calculate_gas_levels(raw_mq):
    ...
    gas_levels = {
        ...
        "so2": round(0.01 + (mq_ratio * 0.1 * time_factor) + np.random.uniform(-0.005, 0.005), 3),  # 0.005-0.15 ppm
...
def update_data(raw_mq):
    ...
    if raw_mq < 250: 
        ...
        # Formula: AQI = 90 + ((raw_mq - 250) / (449 - 250)) * 10
        aqi = int(90 + ((raw_mq - 250) / (449 - 250)) * 10)
